          ...
          <table class="w-full text-left min-w-full">
            ...
            <tbody>
              ...
              >
                ...
                <td class="py-3 px-5 text-sm text-gray-700 whitespace-nowrap">{{ item.nip }}</td>
                <td class="py-3 px-5 text-sm text-gray-700 whitespace-nowrap">{{ item.nama_mapel || "N/A" }}</td>
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              class="border border-gray-300 p-2 w-full rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            <p v-if="errors.nip" class="text-red-600 text-sm mt-1">
              {{ errors.nip }}
            </p>
          </div>
          <div>
...
export default {
  ...
  computed: {
    ...
    filteredData() {
      ...
      return this.guru.filter(item => 
        item.nama.toLowerCase().includes(lowerQuery) || 
        item.nip.toLowerCase().includes(lowerQuery)
      );
    },
  ...
  methods: {
    ...
    async save() {
      ...
      try {
        if (this.isEdit) {
          await axios.put(`${BASE_URL}/api/guru/${this.form.id}`, this.form, this.getAuthConfig());
          this.$refs.toast.show("Guru berhasil diperbarui!", "success");
        } else {
          await axios.post(`${BASE_URL}/api/guru`, this.form, this.getAuthConfig());
          this.$refs.toast.show("Guru berhasil ditambahkan!", "success");
        }
        this.showForm = false;
        this.getData();
      } catch (e) {
    ...
    async remove(id) {
      ...
      try {
        await axios.delete(`${BASE_URL}/api/guru/${id}`, this.getAuthConfig());
        this.$refs.toast.show("Guru berhasil dihapus!", "success");
        this.getData();
      } catch (e) {
