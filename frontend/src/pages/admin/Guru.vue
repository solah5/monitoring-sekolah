      ...
      <div class="relative z-50 bg-[#0057A8] h-full flex flex-col">
        ...
        <nav class="flex-1 w-full mt-10 px-4">
          <ul class="space-y-3">
            ...
            <li>
              <router-link to="/admin/guru" class="nav-link nav-link-active">
  ...
  methods: {
    ...
    async save() {
      ...
      try {
        ...
        if (this.isEdit) {
          await axios.put(`${BASE_URL}/api/guru/${this.form.id}`, this.form, auth);
          this.$refs.toast.show("Guru berhasil diperbarui!", "success");
        } else {
          await axios.post(`${BASE_URL}/api/guru`, this.form, auth);
