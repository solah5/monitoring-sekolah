  ...
  <div class="min-h-screen bg-gray-50 flex">
    ...
    <div class="flex-1 flex flex-col h-screen overflow-y-auto">
      ...
      <main class="flex-1 p-4 md:p-8">
        ...
        >
          <div
            class="bg-gradient-to-br from-purple-100 to-indigo-100 p-6 rounded-xl shadow-md border border-purple-200"
          >
          ...
          <div
            class="bg-gradient-to-br from-orange-100 to-amber-100 p-6 rounded-xl shadow-md border border-orange-200"
          >
            <div class="flex items-center justify-between">
              <h3 class="font-medium text-gray-700 text-sm">Guru BK</h3>
              <ClipboardDocumentCheckIcon class="w-6 h-6 text-orange-600" />
            </div>
            <p class="text-4xl font-bold mt-2 text-orange-900">
              {{ stats.guruBk }}
            </p>
          </div>

          <div
            class="bg-gradient-to-br from-green-100 to-emerald-100 p-6 rounded-xl shadow-md border border-green-200"
          >
            <div class="flex items-center justify-between">
          ...
          <div
            class="bg-gradient-to-br from-blue-100 to-cyan-100 p-6 rounded-xl shadow-md border border-blue-200"
          >
            <div class="flex items-center justify-between">
        ...
        <section class="bg-white rounded-xl p-6 shadow-md border border-gray-200">
          ...
          <div class="overflow-x-auto">
            <div class="min-w-full">
              <div
                class="hidden md:grid grid-cols-12 gap-4 text-xs font-semibold text-gray-500 uppercase px-4 pb-2 border-b"
              >
              ...
              <ul v-if="aktivitas.length > 0" class="space-y-2 mt-2">
                <li
                  v-for="log in aktivitas"
                  :key="log.id"
                  class="grid grid-cols-1 md:grid-cols-12 gap-2 md:gap-4 items-center p-4 rounded-lg hover:bg-gray-50"
                >
...
// Menggunakan URL dari Environment Variable untuk Deployment
  ...
  methods: {
    ...
    async fetchStats() {
      try {
        ...
        const [guruRes, guruBkRes, siswaRes, mapelRes, aktivitasRes] =
          await Promise.all([
            axios.get(`${BASE_URL}/api/guru`, authConfig),
            axios.get(`${BASE_URL}/api/guru_bk`, authConfig),
            axios.get(`${BASE_URL}/api/siswa`, authConfig),
            axios.get(`${BASE_URL}/api/mapel`, authConfig),
            axios.get(`${BASE_URL}/api/aktivitas`, authConfig),
          ]);

        this.stats.guru = guruRes.data.length;
        this.stats.guruBk = guruBkRes.data.length;
        this.stats.siswa = siswaRes.data.length;
        this.stats.mapel = mapelRes.data.length;

        this.aktivitas = aktivitasRes.data;
      } catch (e) {
