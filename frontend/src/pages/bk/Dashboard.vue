    ...
    <div class="flex-1 flex flex-col h-screen overflow-y-auto">
      <header class="sticky top-0 bg-white border-b border-gray-200 z-30">
        <div class="px-4 md:px-8 h-16 flex items-center justify-between">
          ...
          <div class="hidden md:block"><h1 class="text-lg font-semibold text-gray-700">Dashboard BK</h1></div>
        ...
        <section class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
          ...
          <div class="bg-blue-50 p-6 rounded-xl shadow-sm border border-blue-200">
            <div class="flex items-center justify-between">
                <h3 class="font-medium text-blue-800 text-sm">Total Siswa</h3>
                <UserGroupIcon class="w-6 h-6 text-blue-600"/>
            </div>
            <p class="text-4xl font-bold mt-2 text-blue-900">{{ stats.total_siswa }}</p>
          </div>
          ...
          <div class="bg-green-50 p-6 rounded-xl shadow-sm border border-green-200">
            ...
            <p class="text-4xl font-bold mt-2 text-green-900">{{ stats.siswa_eligible }}</p>
          </div>
          ...
          <div class="bg-yellow-50 p-6 rounded-xl shadow-sm border border-yellow-200">
            ...
            <p class="text-4xl font-bold mt-2 text-yellow-900">{{ stats.total_prestasi }}</p>
          </div>
...
import { HomeIcon, ChartBarIcon, TrophyIcon, ArrowLeftOnRectangleIcon, Bars3Icon, XMarkIcon, UserGroupIcon, CheckBadgeIcon } from "@heroicons/vue/24/solid";

const BASE_URL = import.meta.env.VITE_API_URL;

export default {
  components: { Avatar, HomeIcon, ChartBarIcon, TrophyIcon, ArrowLeftOnRectangleIcon, Bars3Icon, XMarkIcon, UserGroupIcon, CheckBadgeIcon },
  data() {
    return {
      openSidebar: false,
  ...
  methods: {
    ...
    async getStats() {
      try {
        const { data } = await axios.get(`${BASE_URL}/api/bk/dashboard`, {
