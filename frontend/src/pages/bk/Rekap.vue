  ...
  <div class="min-h-screen bg-gray-50 flex font-sans">
    ...
    <div class="flex-1 flex flex-col h-screen overflow-y-auto">
      <header class="sticky top-0 bg-white border-b border-gray-200 z-30">
        <div class="px-4 md:px-8 h-16 flex items-center justify-between">
          ...
          <div class="hidden md:block"><h1 class="text-lg font-semibold text-gray-700">Rekapitulasi Nilai Siswa</h1></div>
        ...
        <div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-x-auto">
          <table class="w-full text-left min-w-full">
            <thead class="bg-gray-50 border-b border-gray-200">
              <tr>
                <th class="py-3 px-5 text-sm font-semibold text-gray-600">Peringkat</th>
                <th class="py-3 px-5 text-sm font-semibold text-gray-600">Nama Siswa</th>
                <th class="py-3 px-5 text-sm font-semibold text-gray-600">Kelas</th>
                <th class="py-3 px-5 text-sm font-semibold text-gray-600 text-center">Rata-rata</th>
                <th class="py-3 px-5 text-sm font-semibold text-gray-600 text-center">Prestasi</th>
                <th class="py-3 px-5 text-sm font-semibold text-gray-600 text-center">Status Eligible</th>
                <th class="py-3 px-5 text-sm font-semibold text-gray-600 text-center">Aksi</th>
              </tr>
            </thead>
            <tbody>
              <tr v-if="paginatedData.length === 0"><td colspan="7" class="py-6 text-center text-gray-500">{{ searchQuery ? 'Siswa tidak ditemukan.' : 'Memuat data...' }}</td></tr>
              <tr v-for="(item, index) in paginatedData" :key="item.id" class="border-b hover:bg-gray-50">
                <td class="py-3 px-5 text-center font-bold text-gray-500">#{{ (currentPage - 1) * itemsPerPage + index + 1 }}</td>
                <td class="py-3 px-5 text-sm text-gray-700">{{ item.nama }} <br /><span class="text-xs text-gray-400">{{ item.nis }}</span></td>
                
                <td class="py-3 px-5 text-sm text-gray-700 font-medium">
                    {{ item.kelas }} <span class="text-gray-500 font-normal">- {{ item.rombel }}</span>
                </td>

                <td class="py-3 px-5 text-sm font-bold text-center text-blue-700">{{ parseFloat(item.rata_rata).toFixed(2) }}</td>
                <td class="py-3 px-5 text-center"><span class="bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full text-xs font-bold">{{ item.jumlah_prestasi }}</span></td>
                <td class="py-3 px-5 text-center">
                  <span :class="{
                      ...
                      'bg-gray-100 text-gray-800': !item.status_eligible || item.status_eligible === 'Pending',
                    }" class="px-2 py-1 rounded text-xs font-semibold">
                    {{ item.status_eligible || 'Pending' }}
                  </span>
                </td>
                <td class="py-3 px-5 text-center whitespace-nowrap">
                  <button @click="openDetail(item)" class="text-blue-600 hover:underline text-sm mr-3 font-medium">Detail Nilai</button>
                  <button @click="openEligible(item)" class="text-purple-600 hover:underline text-sm font-medium">Set Status</button>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    <div v-if="showDetail" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[100] p-4" @click.self="showDetail = false">
      <div class="bg-white w-full max-w-2xl p-6 rounded-xl shadow-xl relative max-h-[90vh] overflow-y-auto">
        ...
        <h3 class="text-xl font-bold mb-2 text-gray-800">{{ selectedSiswa?.nama }}</h3>
        ...
        <div class="border rounded-lg overflow-hidden mb-6">
          <table class="w-full text-sm text-left">
            ...
            <tbody>
              <tr v-for="n in detailData.nilai" :key="n.id" class="border-t">
                <td class="p-2">{{ n.nama_mapel }}</td>
                <td class="p-2">{{ n.jenis_nilai }}</td>
                <td class="p-2 font-bold text-gray-800">{{ n.nilai }}</td>
              </tr>
              <tr v-if="detailData.nilai.length === 0"><td colspan="3" class="p-2 text-center text-gray-500">Belum ada nilai</td></tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="border rounded-lg overflow-hidden">
          <table class="w-full text-sm text-left">
            ...
            <tbody>
              <tr v-for="p in detailData.prestasi" :key="p.id" class="border-t">
                <td class="p-2">{{ p.judul_prestasi }}</td>
                <td class="p-2">{{ p.tingkat }}</td>
                <td class="p-2">{{ p.tahun }}</td>
              </tr>
              <tr v-if="detailData.prestasi.length === 0"><td colspan="3" class="p-2 text-center text-gray-500">Belum ada prestasi</td></tr>
            </tbody>
          </table>
        </div>
      </div>
    ...
    <div v-if="showEligible" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[100] p-4" @click.self="showEligible = false">
      <div class="bg-white w-full max-w-sm p-6 rounded-xl shadow-xl relative">
        <button @click="showEligible = false" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600"><XMarkIcon class="w-6 h-6" /></button>
        <h3 class="text-lg font-bold mb-4 text-gray-800">Set Status Eligible</h3>
        <p class="mb-4 text-sm text-gray-600">Tentukan status untuk <b>{{ selectedSiswa?.nama }}</b>:</p>
        <div class="space-y-2">
          <button @click="setStatus('Eligible')" class="w-full bg-green-100 text-green-800 py-2 rounded font-semibold hover:bg-green-200 transition">Eligible (Layak)</button>
          <button @click="setStatus('Tidak Eligible')" class="w-full bg-red-100 text-red-800 py-2 rounded font-semibold hover:bg-red-200 transition">Tidak Eligible</button>
          <button @click="setStatus('Pending')" class="w-full bg-gray-100 text-gray-800 py-2 rounded font-semibold hover:bg-gray-200 transition">Pending</button>
        </div>
      </div>
    </div>
