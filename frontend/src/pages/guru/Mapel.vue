      ...
      <div class="relative z-50 bg-[#0057A8] h-full flex flex-col">
        ...
        <nav class="flex-1 w-full mt-10 px-4">
          <ul class="space-y-3">
            ...
            <li>
              <router-link to="/guru/mapel" class="nav-link nav-link-active">
      ...
      <div class="bg-white w-full max-w-md p-6 rounded-xl shadow-xl relative">
        ...
        <div class="space-y-4">
          <div>
            ...
            />
            <p v-if="errors.nama_mapel" class="text-red-600 text-sm mt-1">
              {{ errors.nama_mapel }}
            </p>
          </div>
        </div>
...
const API_URL = `${BASE_URL}/api/mapel`;
...
export default {
  ...
  data() {
    return {
      ...
      showForm: false,
      isEdit: false,
      form: { id: null, nama_mapel: "" },
      errors: { nama_mapel: "" },
    };
  },
  computed: {
    filteredData() {
      if (!this.searchQuery) return this.mapel;
      const q = this.searchQuery.toLowerCase();
      return this.mapel.filter((item) =>
  ...
  methods: {
    ...
    openForm() {
      this.form = { id: null, nama_mapel: "" };
      this.errors.nama_mapel = "";
      this.isEdit = false;
      this.showForm = true;
    },
    edit(item) {
    ...
    validate() {
      ...
      if (!this.form.nama_mapel) {
        this.errors.nama_mapel = "Nama mapel wajib diisi";
        return false;
      }
      return true;
    },
    async save() {
      ...
      try {
        if (this.isEdit) {
          ...
          this.$refs.toast.show("Mapel berhasil diperbarui!", "success");
        } else {
          await axios.post(API_URL, this.form, this.getAuthConfig());
          this.$refs.toast.show("Mapel berhasil ditambahkan!", "success");
        }
        this.showForm = false;
        this.getData();
      } catch (e) {
    ...
    async remove(id) {
      if (!confirm("Yakin hapus? Ini akan menghapus mapel dari guru terkait."))
        return;
      try {
        await axios.delete(`${API_URL}/${id}`, this.getAuthConfig());
        this.$refs.toast.show("Mapel berhasil dihapus!", "success");
        this.getData();
      } catch (e) {
