  ...
  <div class="min-h-screen bg-gray-50 flex font-sans">
    ...
    >
      ...
      <div class="relative z-50 bg-[#0057A8] h-full flex flex-col">
        <button @click="openSidebar = false" class="md:hidden absolute top-4 right-4 p-1 text-white">
      ...
      <header class="sticky top-0 bg-white border-b border-gray-200 z-30 shadow-sm">
        <div class="px-4 md:px-8 h-16 flex items-center justify-between">
          ...
          <div class="hidden md:block">
            <h1 class="text-lg font-semibold text-gray-700">Input Nilai Siswa (Wali Kelas)</h1>
          </div>
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mb-6">
          ...
          <div class="flex flex-col md:flex-row gap-4 items-start md:items-center">
            ...
            <span class="text-sm text-gray-500 italic bg-blue-50 px-3 py-1 rounded-md text-blue-700">
              *Mengganti mapel akan menampilkan nilai siswa untuk mapel tersebut
            </span>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      mapelList: [],
      siswaList: [],
      allNilaiList: [], // Menampung SEMUA data nilai dari DB untuk dicocokkan
      selectedMapelId: "",
      inputNilai: {}, 
      loadingId: null,
    };
  ...
  methods: {
    ...
    async getData() {
      ...
      try {
        const [siswaRes, mapelRes, nilaiRes] = await Promise.all([
          axios.get(`${BASE_URL}/api/siswa`, auth),
          axios.get(`${BASE_URL}/api/mapel`, auth),
          axios.get(`${BASE_URL}/api/nilai`, auth) // Ambil semua nilai untuk referensi
        ]);
        
        this.siswaList = siswaRes.data;
        this.mapelList = mapelRes.data;
        this.allNilaiList = nilaiRes.data; 

        // Inisialisasi awal form (kosong)
        this.resetForm();
    ...
    // 1. Fungsi RESET: Mengosongkan semua input
    resetForm() {
    ...
    // 2. Fungsi UPDATE: Dipanggil saat Dropdown Mapel berubah
    updateFormNilai() {
       // Langkah A: Kosongkan dulu semua input agar bersih
       this.resetForm();

       // Langkah B: Jika ada mapel yg dipilih, cari apakah siswa sudah punya nilai di mapel itu
       if (this.selectedMapelId) {
         this.siswaList.forEach(s => {
            // Cari data di allNilaiList yang cocok (Siswa ID & Mapel ID sama)
            const existing = this.allNilaiList.find(n => 
                n.siswa_id === s.id && n.mapel_id === this.selectedMapelId
            );

            // Jika ketemu, isi form dengan nilai dari database
            if (existing) {
    ...
    hitungAkhir(siswaId) {
        ...
        // Jika form kosong semua, tampilkan strip
    ...
    async simpanNilai(siswaId) {
      ...
      try {
        ...
        // Refresh data nilai di background agar data terbaru tersimpan di memori
        const nilaiRes = await axios.get(`${BASE_URL}/api/nilai`, this.getAuth());
        this.allNilaiList = nilaiRes.data;
